 ...
 */
class js_object : public js_data
{
public:
    js_object();
    ...
    js_result get_object(const std::string& tag, js_object*& object) const;

    js_result get_bool(const std::string& tag, bool& b) const;

    js_result get_number(const std::string& tag, double& d) const;

    js_result get_string(const std::string& tag, std::string& str) const;

    js_result get_array(const std::string& tag, js_array*& array) const;

    js_object* add_object(js_root* root, const std::string& tag);
    ...
    js_array* add_array(js_root* pool, const std::string& tag);

    void add(const std::string& tag, js_data* data);
private:
    std::unordered_map<std::string, js_data*> map;
};
